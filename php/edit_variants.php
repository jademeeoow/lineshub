...
if (!$variant_id) {
    ...
    $redirect_url = "../templates/html/variants.php?message2=" . urlencode($message);
    if ($category_id) {
...
try {
    ...
    $stmt->bind_param("sssiisi", $variant_color, $variant_size, $variant_price, $variant_stock, $variant_stock, $supplier, $variant_id);
    if (!$stmt->execute()) {
    ...
    if ($product_name && $product_description) {
        $stmt = $conn->prepare("
            UPDATE lines_products 
            SET product_name = ?, product_description = ?
            WHERE product_id = (SELECT product_id FROM lines_product_variants WHERE variant_id = ?)
        ");
        if (!$stmt) {
            throw new Exception("Prepare failed: " . $conn->error);
        }
    ...
    $redirect_url = "../templates/html/variants.php?message=" . urlencode($message);
    if ($category_id) {
        $redirect_url .= "&category_id=" . urlencode($category_id);
    }
    header("Location: " . $redirect_url);
