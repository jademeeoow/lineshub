...
function getVariants($conn, $category_id)
{
    ...
    $query = "SELECT 
        ...
        COALESCE(ls.sub_category_name, 'None') AS sub_category_name,
        COALESCE(s.contact_person, 'N/A') AS supplier_name
      FROM 
        lines_product_variants lv
      JOIN 
        lines_products p ON lv.product_id = p.product_id
      LEFT JOIN 
        lines_sub_category ls ON p.sub_category_id = ls.sub_category_id
      LEFT JOIN 
...
{
    $query = "SELECT product_id FROM lines_products WHERE product_name = ? AND product_description = ?";
    $stmt = $conn->prepare($query);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['add_variant'])) {
        ...
        if (!$product_id) {
            ...
            header("Location: variants.php?category_id=" . $category_id . "&status=error");
            exit();
        } else {
...
foreach ($variants as $variant) {
    ...
    $insertQuery = "INSERT INTO lines_product_variants 
    ...
    if ($stmt) {
        ...
        if ($stmt->execute()) {
            ...
            error_log("Error adding variant: " . $stmt->error); // Log error
        }
        $stmt->close();
    } else {
        $_SESSION['message2'] = "Error preparing statement: " . $conn->error;
        ...
        header("Location: variants.php?category_id=" . $category_id . "&status=success");
        exit();
    }
}
...
function getOrCreateProduct($conn, $product_name, $product_description, $subcategory_id, $category_id)
{
    ...
    if (!empty($product_name) && !empty($product_description)) {
        ...
        $query = "SELECT product_id FROM lines_products 
        ...
        $stmt->close();
        $insertQuery = "INSERT INTO lines_products 
...
if (isset($_GET['delete_variant'])) {
    ...
    if ($variant_id > 0) {
        $deleteQuery = "DELETE FROM lines_product_variants WHERE variant_id = ?";
...
{
    $query = "SELECT category_name FROM lines_category WHERE category_id = ?";
    $stmt = $conn->prepare($query);
